...
To save data entered by the user, I\'ve been using the \href{https://docs.python.org/3.6/library/shelve.html}{shelve} module. This results in several data files: \green{IngredientShelf.dat}, \green{OtherShelf.dat}, and \green{OxideShelf.dat} (these all have associated bak and dir files). Derek Au is in the process of switching to JSON instead. The JSON files will be stored in the `data' folder.
...
\item If you edit the oxide composition of an ingredient that has been selected, and then click `Update', the list of oxides shown doesn't change to reflect this.
\item If you create a new ingredient, calculate the restrictions, delete the new ingredient, create a new ingredient (we're assuming it has the same name as the previous one), and try calculate the restrictions again, an error message appears saying, in part, `pulp.constants.PulpError: Repeated variable names in Lp format'. This was not a problem with previous versions of LIPGLOSS.
